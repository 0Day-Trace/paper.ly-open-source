...
export default function RotatePdf({ onBack, tool, onComplete }) {
  ...
  const loadThumbnails = async (f) => {
    ...
    try {
      ...
      countForm.append('user_id', getUserId())
      const countRes = await axios.post(`${API_BASE}/pagecount`, countForm, { timeout: 15000 })
      ...
      for (let start = 1; start < total; start += BATCH) {
        ...
        for (let i = start; i < Math.min(start + BATCH, total); i++) {
          ...
          batch.push(axios.post(`${API_BASE}/page-thumbnail`, pf, { timeout: 15000 }).then(r => ({ i, thumb: r.data.thumbnail })).catch(() => ({ i, thumb: null })))
        }
