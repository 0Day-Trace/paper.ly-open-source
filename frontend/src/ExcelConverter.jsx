...
export default function ExcelConverter({ onBack, tool, onComplete }) {
  ...
  const accent = resolveAccent(tool?.accent || '#16A34A', theme)
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/excel-convert`, formData)
  ...
  return (
    <ToolShell tool={tool} onBack={onBack} loading={loading} loadingLabel="Converting your spreadsheet">
        <DropZone
          ...
          accept=".xlsx,.xls,.csv"
          acceptLabel="XLSX, XLS and CSV files"
          accent={accent}
          id="excel-input"
        />
