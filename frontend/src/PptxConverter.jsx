...
export default function PptxConverter({ onBack, tool, onComplete }) {
  ...
  const accent = resolveAccent(tool?.accent || '#C2410C', theme)
  ...
  const handleConvert = async () => {
    ...
    formData.append('output', outputFmt === 'pdf' ? 'pdf' : 'image')
    if (outputFmt !== 'pdf') formData.append('img_fmt', outputFmt)
    formData.append('user_id', getUserId())
    try {
      const response = await axios.post(`${API_BASE}/pptx-convert`, formData)
  ...
  return (
    <ToolShell tool={tool} onBack={onBack} loading={loading} loadingLabel="Converting your presentation">
        <DropZone
          file={file}
          onFile={f => { setFile(f); setError('') }}
          accept=".pptx,.ppt"
          acceptLabel="PPTX and PPT files"
          accent={accent}
          id="pptx-input"
        />
