...
export default function ImageToPdfTool({ onBack, tool, onComplete }) {
  ...
  return (
    <ToolShell tool={tool} onBack={onBack} loading={loading} loadingLabel="Building your PDF">
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: 16, boxShadow: 'var(--shadow-sm)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 'clamp(11px, 2.5vw, 12px)', color: 'var(--text-3)' }}>Paper size<select value={paperSize} onChange={(e) => setPaperSize(e.target.value)} style={{...selectStyle, fontSize: 'clamp(12px, 3vw, 14px)', padding: 'clamp(8px, 2vw, 10px) clamp(32px, 7vw, 36px) clamp(8px, 2vw, 10px) clamp(10px, 2.5vw, 12px)'}}><option value="fit">Fit (default)</option><option value="a4">A4</option><option value="a3">A3</option><option value="a2">A2</option><option value="letter">Letter</option></select></label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 'clamp(11px, 2.5vw, 12px)', color: 'var(--text-3)' }}>Orientation<select value={orientation} onChange={(e) => setOrientation(e.target.value)} style={{...selectStyle, fontSize: 'clamp(12px, 3vw, 14px)', padding: 'clamp(8px, 2vw, 10px) clamp(32px, 7vw, 36px) clamp(8px, 2vw, 10px) clamp(10px, 2.5vw, 12px)'}}><option value="portrait">Portrait</option><option value="landscape">Landscape</option></select></label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 'clamp(11px, 2.5vw, 12px)', color: 'var(--text-3)' }}>Margin<select value={margin} onChange={(e) => setMargin(e.target.value)} style={{...selectStyle, fontSize: 'clamp(12px, 3vw, 14px)', padding: 'clamp(8px, 2vw, 10px) clamp(32px, 7vw, 36px) clamp(8px, 2vw, 10px) clamp(10px, 2.5vw, 12px)'}}><option value="none">None</option><option value="small">Small (10mm)</option><option value="large">Large (20mm)</option></select></label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 'clamp(11px, 2.5vw, 12px)', color: 'var(--text-3)' }}>Output<select value={outputMode} onChange={(e) => setOutputMode(e.target.value)} style={{...selectStyle, fontSize: 'clamp(12px, 3vw, 14px)', padding: 'clamp(8px, 2vw, 10px) clamp(32px, 7vw, 36px) clamp(8px, 2vw, 10px) clamp(10px, 2.5vw, 12px)'}}><option value="merged">1 PDF</option><option value="separate">ZIP (one PDF per image)</option></select></label>
        </div>
